...
__s32 get_audio_info(__s32 sample_rate)
{
	...
	vic_tab = get_video_info(glb_video_para.vic);
	if (vic_tab == -1)
		return -1;

	/*FIXME: not sure about 4K resolutions */
	dw = video_timing[vic_tab].pixel_repeat;
	audio_info.CTS = (((video_timing[vic_tab].pixel_clk << dw)  / 100) *
...
__s32 set_video_enable(bool enable)
{
	...
	{
		__s32 vic_tab = get_video_info(glb_video_para.vic);
		if(vic_tab == -1) {
			ret = -1;
			goto video_en_end;
		}
		...
		if(bsp_hdmi_video(&glb_video_para, &video_timing[vic_tab]))
		{
